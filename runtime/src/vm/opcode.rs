...
pub static OPCODES: Lazy<HashMap<i64, ByteOpcode>> = Lazy::new(|| {
  ...
  // fits in the "address space" for all of them
  cpu!("seti64", |args, hand_mem, _| {
    let data = args[0];
    hand_mem.write(args[1], 8, &data.to_le_bytes());
    None
  });
  cpu!("seti32", |args, hand_mem, _| {
    let data = args[0] as i32;
    hand_mem.write(args[1], 4, &data.to_le_bytes());
    None
  });
  cpu!("seti16", |args, hand_mem, _| {
    let data = args[0] as i16;
    hand_mem.write(args[1], 2, &data.to_le_bytes());
    None
  });
  cpu!("seti8", |args, hand_mem, _| {
    let data = args[0] as i8;
    hand_mem.write(args[1], 1, &data.to_le_bytes());
    None
  });
