...
opcodeScope.put(
  'any',
  new Type('any', true, false, {}, {}, null, new Interface('any')),
);
opcodeScope.put(
  'anythingElse',
  new Type(
...
Type.builtinTypes.Result.solidify(['int16'], opcodeScope);
Type.builtinTypes.Result.solidify(['int32'], opcodeScope);
...
opcodeScope.put(
  '__conn',
  new Event('__conn', Type.builtinTypes.InternalRequest, true),
);
opcodeScope.put(
  '__ctrl',
  new Event('__ctrl', Type.builtinTypes.InternalRequest, true),
);
opcodeScope.put(
  'tcpConn',
  new Event('tcpConn', Type.builtinTypes.TcpChannel, true),
);
opcodeScope.put(
  'chunk',
  new Event('chunk', opcodeScope.get('TcpContext<any>') as Type, true),
);
opcodeScope.put(
  'tcpClose',
  new Event('tcpClose', opcodeScope.get('TcpContext<any>') as Type, true),
);
              ...
              ((inputTypes, scope) => {
                ...
                if (returnType.iface) {
                  ...
                  Object.values(args).forEach((a: Type, i: number) => {
                      ...
                      if (
                        ...
                        try {
                          ...
                          try {
                            ...
                            const closureArgs = Object.values(
                              fn.getArguments(),
                            ) as Type[];
                            closureArgs[0].typeApplies(
                              arrayInnerType,
                              scope,
                              interfaceMap,
                            );
                            closureArgs[1].typeApplies(
                        ...
                        const reducerTypes = Object.values(
                          inputTypes[0].properties,
                        ) as Type[];
                        const inType = scope.deepGet(
                          reducerTypes[0].typename.replace(
                            /^Array<(.*)>$/,
                            '$1',
                          ),
                        ) as Type;
                    ...
                    if (
                      Object.values(a.properties).some(
                        (p) =>
                          !!p.iface &&
                      ...
                      Object.values(a.properties).forEach((p, j) => {
                        if (
                          ...
                          replacementType = Object.values(
                            inputTypes[i].properties,
                          )[j] as Type;
                        }
                      });
                    }
                  ...
                  Object.values(returnType.properties).some(
                    (p: Type) => !!p.iface,
                  )
                ) {
                  // TODO: Remove this hackery after function types are more than just 'function'
                  if (
                    ...
                    const arrayInnerTypeStr = inputTypes[0].typename.replace(
                      /^Array<(.*)>$/,
                      '$1',
                    );
                    const arrayInnerType = arrayInnerTypeStr.includes('<')
                      ? (() => {
                          const fulltypenameAst =
                            fulltypenameAstFromString(arrayInnerTypeStr);
                    ...
                    if (['filter', 'filterl'].includes(opcodeName)) {
                      ...
                      const newInnerType = innerType.realize(
                        interfaceMap,
                        scope,
                      ); // Necessary?
                    ...
                    return inputs[0].inputNames[1]
                      ? Microstatement.fromVarName(
                          inputs[0].inputNames[1],
                          scope,
                          microstatements,
                        ).closureOutputType
                      : returnType;
                    ...
                    const returnIfaces = Object.values(returnType.properties)
                      .filter((p: Type) => !!p.iface)
                      .map((p: Type) => p.iface);
                    if (returnIfaces.length > 0) {
                      const newReturnType = returnType.realize(
                        interfaceMap,
                        scope,
                      );
                      return newReturnType;
                        ...
                        const arrayInnerType = arrayInnerTypeStr.includes('<')
                          ? (() => {
                              const fulltypenameAst =
                                fulltypenameAstFromString(arrayInnerTypeStr);
                              const baseTypeStr =
                                fulltypenameAst.get('typename').t;
                              const baseType = scope.deepGet(
                                baseTypeStr,
                              ) as Type;
                              const generics = [
                                fulltypenameAst
