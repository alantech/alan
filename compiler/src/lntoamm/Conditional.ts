...
export class Conditional {
  static isNested(microstatements: Microstatement[]): boolean {
  ...
  static handleTail(
    ...
    const isNested = Conditional.isNested(microstatements);
    ...
    if (!isNested && (closureMstmts[closureMstmts.length - 1].statementType !== StatementType.EXIT)) {
      Conditional.returnVoid(closureMstmts, tail.scope);
    }
    ...
    const isUnwrapReturn = tail.isUnwrapReturn; // TODO: determine if this is even necessary, or if it should replace isNested
  ...
  static returnVoid(microstatements: Microstatement[], scope: Scope) {
...
const unwrapCondval: Fn = {
  ...
  getArguments: () => ({} as Args),
  getReturnType: () => Type.builtinTypes.void, // TODO: replace this once it can be consistently determined
  isPure: () => true,
  isUnwrapReturn: () => false,
  microstatementInlining: (realArgNames: string[], scope: Scope, microstatements: Microstatement[]) => {
    microstatements.push(new Microstatement(
      ...
      true,
      realArgNames.pop(),
      Type.builtinTypes.void, // TODO: replace this once it can be consistently determined
      [...realArgNames],
      [unwrapCondval],
    ));
  },
