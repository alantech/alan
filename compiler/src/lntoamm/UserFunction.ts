...
${statements[i].statementAst.t.trim()} on line ${
          statements[i].statementAst.line
        }:${statements[i].statementAst.char}`);
      }
    }
  ...
  static fromAst(functionishAst: LPNode, scope: Scope) {
    if (
      ...
      if (functionishAst.has('functions')) {
        return UserFunction.fromFunctionsAst(
          functionishAst.get('functions'),
          scope,
        );
      }
      if (functionishAst.has('functionbody')) {
        return UserFunction.fromFunctionbodyAst(
          functionishAst.get('functionbody'),
          scope,
        );
      }
  ...
  static fromFunctionsAst(functionAst: LPNode, scope: Scope) {
      ...
      for (let i = 0; i < argsArr.length; i++) {
        ...
        if (!getArgType) {
          if (argsArr[i].fulltypename.has('opttypegenerics')) {
            getArgType = scope.deepGet(
              argsArr[i].fulltypename.get('typename').t,
            ) as Type;
            if (!getArgType) {
              throw new Error(
                'Could not find type ' +
            ...
            if (!(getArgType instanceof Type)) {
              throw new Error(
                'Function argument is not a valid type: ' +
                  argsArr[i].fulltypename.t,
              );
            }
            const genericTypes = [];
    ...
    if (functionAst.get('fullfunctionbody').has('functionbody')) {
      ...
      const assignablesAst = functionAst
        ...
        .get('assignfunction')
        .get('assignables');
      const statementAst = Ast.statementAstFromString(
        `return ${assignablesAst.t};`,
      );
      ...
      if (getReturnType == null || !(getReturnType instanceof Type)) {
        if (fulltypename.has('opttypegenerics')) {
          ...
          if (!(getReturnType instanceof Type)) {
            throw new Error(
              'Function return is not a valid type: ' + fulltypename.t,
            );
          }
          const genericTypes = [];
          genericTypes.push(
  ...
  static conditionalToCond(cond: LPNode, scope: Scope) {
    const newStatements: Array<LPNode> = [];
    ...
    const condBlockFn = (
      cond.get('blocklike').has('functionbody')
        ? UserFunction.fromFunctionbodyAst(
            cond.get('blocklike').get('functionbody'),
            scope,
          )
        ...
        : UserFunction.fromFunctionsAst(
            cond.get('blocklike').get('functions'),
            scope,
          )
    ).maybeTransform(new Map(), scope);
    if (
      condBlockFn.statements[
        condBlockFn.statements.length - 1
      ].isReturnStatement()
    ) {
    ...
    if (cond.has('elsebranch')) {
      ...
      if (notcond.get('condorblock').has('blocklike')) {
        ...
        const elseBlockFn = (
          notblock.has('functionbody')
            ? UserFunction.fromFunctionbodyAst(
                notblock.get('functionbody'),
                scope,
              )
            : notblock.has('fnname')
            ? // TODO: If more than one function matches, need to run multiple dispatch logic
        ...
        if (
          elseBlockFn.statements[
            elseBlockFn.statements.length - 1
          ].isReturnStatement()
        ) {
          hasConditionalReturn = true;
        }
      ...
      if (
        ...
        if (args.length == 2) {
          ...
          if (block) {
            ...
            if (
              blockFn.statements[
                blockFn.statements.length - 1
              ].isReturnStatement()
            ) {
              const innerStatements = blockFn.statements.map(
                (s) => s.statementAst,
              );
    ...
    if (replacementStatements[replacementStatements.length - 1].has('exits')) {
      ...
      if (retStatement.get('exits').get('retval').has('assignables')) {
        const newAssign = Ast.statementAstFromString(
          `
          ${retVal} = ref(${
            retStatement.get('exits').get('retval').get('assignables').t
          })
        `.trim() + ';',
        );
  ...
  maybeTransform(interfaceMap: Map<Type, Type>, scope?: Scope) {
    if (
      ...
      for (let i = 0; i < this.statements.length; i++) {
        ...
        const corrected = str.replace(
          ...
          (
            ...
            const replacementType = originalType.realize(
              interfaceMap,
              newScope,
            );
      ...
      `.trim();
      const fn = UserFunction.fromAst(
        Ast.functionAstFromString(fnStr),
        this.scope,
      );
      return fn;
    } else {
  ...
  microstatementInlining(
    ...
    const enterfns = microstatements.filter(
      (m) => m.statementType === StatementType.ENTERFN,
    );
    ...
    const inputs = realArgNames.map((n) =>
      Microstatement.fromVarName(n, scope, microstatements),
    );
    ...
    if (!this.getReturnType().typeApplies(last.outputType, scope, new Map())) {
      const returnTypeAst = Ast.fulltypenameAstFromString(
        this.getReturnType().typename,
      );
      let returnSubtypes = [];
        ...
        returnSubtypes = returnSubtypeAsts.map((r) => {
          ...
          if (!t) {
            ...
            if (r.has('opttypegenerics')) {
              innerGenerics.push(
                r.get('opttypegenerics').get('generics').get('fulltypename').t,
              );
            ...
            t = (scope.deepGet(r.get('typename').t) as Type).solidify(
              innerGenerics,
              scope,
            );
          }
      ...
      if (this.getReturnType().iface) {
        ...
        if (returnSubtypes.some((t: Type) => !!t.iface)) {
          ...
          const newReturnType = oldReturnType.originalType.solidify(
        ...
        if (lastSubtypes.some((t: Type) => t.hasInterfaceType())) {
          ...
          if (lastSubtypes.some((t: Type) => t.hasInterfaceType())) {
            ...
            const newLastType = oldLastType.originalType.solidify(
  ...
  static dispatchFn(fns: Array<Fn>, argumentTypeList: Array<Type>, s: Scope) {
    let fn = null;
    ...
    if (fn == null) {
      ...
      for (let i = 0; i < fns.length; i++) {
        ...
        if (fn instanceof UserFunction) {
          ...
          errMsg += `fn ${fn.getName()}(${Object.entries(fn.getArguments()).map(
            (kv) => `${kv[0]}: ${kv[1].typename}`,
          )}): ${fn.getReturnType().typename}\n`;
        }
      }
